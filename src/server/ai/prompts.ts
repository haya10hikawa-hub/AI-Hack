...
const PURPOSE_INSTRUCTIONS: Record<AIPurpose, string> = {
  analyze_sequence: `Return only visible semantic observations for each supplied analysis-candidate derivative: activity, object, people_group, environment, moment, detail, or visible_text. For the activity observation, combine what is visible in the photo with the supplied sequenceMetadata (mediaCount, durationMinutes, timeOfDay) and each asset's capturedAt and coarsePlace to infer the most likely thing the user was doing; the activity must stay grounded in the visible image, using metadata only to disambiguate (e.g. a table set with food photographed in the evening is a meal, not a meeting). Report uncertainty, and include a specific uncertainty for every low-confidence observation. Do not select representative images. Do not infer purpose, personal identity, relationship, emotion, or exact location from appearance alone.`,
